...
internal class EntryWidgetController @JvmOverloads constructor(
    ...
    private val entryWidgetObservableItemType: Flowable<List<EntryWidgetContract.ItemType>>
        get() = Flowable.combineLatest(
            ...
            GliaLogger.i(LogEvents.ENTRY_WIDGET_STATE_CHANGED) {
                put(EventAttribute.ViewType, viewType.toTelemetryViewType())
                val state = itemTypes.toTelemetryEntryWidgetState()
                put(EventAttribute.EntryWidgetState, state)
                if (state == EntryWidgetState.ITEMS) {
                    put(EventAttribute.EntryWidgetItems, itemTypes.toTelemetryEngagementTypes().joinToString(", "))
                } else if(state == EntryWidgetState.ONGOING_ENGAGEMENT) {
                    put(EventAttribute.EngagementType, itemTypes.toTelemetryEngagementTypes().firstOrNull() ?: "N/A")
                }
            }
        }

    private lateinit var view: EntryWidgetContract.View
    private lateinit var viewType: EntryWidgetContract.ViewType
    ...
    private fun EntryWidgetContract.ViewType.toTelemetryViewType(): String? = when (this) {
        ...
        EntryWidgetContract.ViewType.MESSAGING_LIVE_SUPPORT -> null
    }
