...
**Retained controllers** — `retainedChatController` and `retainedCallController` in `ControllerFactory.java` are instance fields, null-checked and lazily constructed. They survive Activity recreation; no `ViewModel` exists because of them. `destroyChatController()` / `destroyCallController()` call `onDestroy(false)` and null the reference. `destroyControllers()` runs both plus `chatBubbleController` and `messagesNotSeenHandler`.

**The bubble is one controller** — `chatBubbleController` (`ChatBubbleController`, cached in a `private static` field) owns all chat-head business logic and publishes a single `BubbleUiModel` via `StateFlow`. Its hosts — `ChatHeadService`, `ChatHeadLayout`, `ActivityWatcherForChatHead` — are pure renderers that collect `uiState`; only the watcher reports screen lifecycle back, via `onContextEvent`. Never add a second bubble controller or let a host decide visibility for itself.

**`ChatHeadService`'s lifetime is not the overlay bubble's visibility** — the service runs whenever `DecideBubbleRenderTargetUseCase.isOverlayServiceNeeded` (screen-independent: overlay allowed and an engagement to return to), and adds or removes its own window as the render target changes. This is why it starts while the app is still foregrounded: API 26+ refuses a background service start, so a service started at `ON_STOP` would be racing the restriction. `ChatBubbleController` re-asserts start/stop on every recompute rather than on a change edge, because `ChatHeadManager` clears `isServiceStarted` when the OS refuses a start — re-asking is the retry. Do not "optimize" that into an edge check.
...
**Activity watchers share a coordination bus** — every watcher registered in `Dependencies.onAppCreate` extends one of two bases in `base/BaseActivityWatcher.kt`, and which one decides what "current activity" means:
- `BaseActivityStackWatcher` keeps the whole resumed stack and exposes `topActivityObserver: Observable<Activity>`. Used by `ActivityWatcherForChatHead`, `ActivityWatcherForLiveObservation`, `ActivityWatcherForPermissionsRequest`. Note its stack is newest-first, so `topActivityObserver` emits the *oldest* resumed activity — which is what keeps the translucent `CallActivity` sitting over another screen resolving to the screen underneath.
- `BaseSingleActivityWatcher(gliaActivityManager)` tracks only the resumed activity and exposes `resumedActivity: Flowable<WeakReference<Activity>>`, emitting a `WeakReference(null)` on pause so a watcher can react to there being no Glia screen at all. Used by `CallVisualizerActivityWatcher`, `EngagementCompletionActivityWatcher`, `OperatorRequestActivityWatcher`, `UiComponentsActivityWatcher`. It also owns dialog hosting (`showAlertDialogWithStyledContext`) and activity finishing.

Route "top activity" access through one of these — never a static field.
