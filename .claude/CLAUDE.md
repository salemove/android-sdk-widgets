...
**Making commits & PRs**: Always use the skills — `/commit` for every commit, `/gh` for every PR — never invoke `git commit` / `gh pr create` by hand outside them, and never `/create-pr` (it ignores the PR template).
**PR body**: Every PR body must be `.github/PULL_REQUEST_TEMPLATE.md` filled in — never a freeform description. Keep the bold section labels (`**Jira issue:**`, `**What was solved?**`, `**Additional info:**`, `**Screenshots:**`) and the `Additional info` checklist verbatim and in order — they are not markdown headings, so do not convert them. Leave checklist boxes unchecked; they are the author's to confirm. Take the Jira key from the branch name (`MOB-1234-...` → `https://glia.atlassian.net/browse/MOB-1234`); if the branch has no key, leave the placeholder URL untouched rather than guessing. Fill `Screenshots:` for every user-visible change — before/after images or a recording — and leave it empty only for non-UI work. Pass the filled body via `--body`/`--body-file`; never `--template` (exits 1 outside a TTY). `--web` is the no-template fallback only — this repo has a template, so it never applies here.
