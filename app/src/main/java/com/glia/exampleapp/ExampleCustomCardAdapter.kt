...
class ExampleCustomCardAdapter : CustomCardAdapter() {
    ...
    override fun shouldShowCard(message: CustomCardMessage, viewType: Int): Boolean {
        if (viewType == NATIVE_VIEW_TYPE) {
            return message.metadata?.optBoolean(NativeViewViewHolder.SHOULD_SHOW_VIEW_KEY) ?: false
        }
    ...
    internal class NativeViewViewHolder(itemView: View) : CustomCardViewHolder(itemView) {
        private val metadataTextView: TextView = itemView.findViewById(R.id.metadata)
        private val okButton: Button = itemView.findViewById(R.id.ok_button)
        ...
        override fun bind(message: CustomCardMessage, callback: ResponseCallback) {
            try {
                ...
                if (message.metadata?.optBoolean(
                        DeprecatedExampleCustomCardAdapter.NativeViewViewHolder.SHOW_BUTTON_KEY, false
                    ) == true
                ) {
        ...
        companion object {
            ...
            fun isNativeViewType(message: CustomCardMessage?): Boolean {
                val metadata = message?.metadata
                return if (metadata == null || metadata.length() == 0) {
                    false
                } else {
                    (metadata.has(SHOW_BUTTON_KEY)
                        || metadata.has(SHOULD_SHOW_VIEW_KEY))
                }
            }
        }
    }
