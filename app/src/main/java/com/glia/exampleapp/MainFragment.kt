...
class MainFragment : Fragment() {
    ...
    private val pushNotificationPermissionLauncher = registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted ->
    ...
    private fun listenForGliaEvents() {
        GliaWidgets.subscribeToEvents { event ->
            activity?.runOnUiThread {
                when (event) {
                    is LifecycleEvent.EngagementStarted -> onEngagementStarted()
                    is LifecycleEvent.EngagementEnded -> onEngagementEnded()
                    else -> {}
                }
