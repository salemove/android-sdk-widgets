...
internal class TextCollectorDialogDetector(val textCollectorHelper: TextCollectorHelper) {
    fun hasDialog(allRootViews: List<View>): Boolean {
        return try {
                ...
                allRootViews.forEach { rootView ->
                    if (rootView.isVisible) {
                        if (isDialogWindow(rootView)) {
                            Logger.d(TextCollector.TAG, "Dialog detected") // TODO: remove if not working
                            return true
                        }
                    }
                }
            ...
            if (hasComposeDialog(allRootViews)) {
                Logger.d(TextCollector.TAG, "Compose dialog detected") // TODO: remove if not working
                return true
            }
        ...
        try {
            ...
            config.forEach { entry ->
                ...
                if (key.contains("Dialog", ignoreCase = true) ||
                    ...
                    return true // TODO: remove if not working
                }
            }
            try {
    ...
    private fun isDialogWindow(rootView: View): Boolean {
        ...
        try {
            ...
            if (rootViewParent != null && rootViewParent.javaClass.name.contains("ViewRootImpl")) {
                val windowAttributesField = rootViewParent.javaClass.getDeclaredField("mWindowAttributes")
                windowAttributesField.isAccessible = true
                val windowAttributes = windowAttributesField.get(rootViewParent)
                if (windowAttributes != null) {
                    val typeField = windowAttributes.javaClass.getDeclaredField("type")
                    typeField.isAccessible = true
                    val type = typeField.getInt(windowAttributes)
                    if (type >= 1000 && type <= 1999) { // TODO: looks bad
                        return true // TODO: remove if not working
                    }
                }
            }
        } catch (e: Exception) {
            Logger.d(TextCollector.TAG, "Exception in isDialogWindow $e")
        }
        ...
        if (layoutParams != null &&
            ...
            if (rootView is android.view.ViewGroup && rootView.childCount > 0) {
        ...
        if (rootView.width > 0 && rootView.height > 0) {
            ...
            if ((widthRatio < 0.95f || heightRatio < 0.95f) && rootView is android.view.ViewGroup && rootView.childCount > 0) {
                return true // TODO: remove if not working
            }
        }
        return false // TODO: remove if not working
    }
